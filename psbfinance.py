...
st.markdown("""
<style>
:root{ --ink:#0e1a2b; --muted:#6b7b91; --card:#f6f8fb; --accent:#0d6efd; }
...
# ---------------------- PORTFOLIO OPTIMIZER ----------------------
    ...
    if run:
        ...
        if prices.empty or len(prices.columns) < 2:
            st.error("Please enter at least two valid tickers for optimization.")
        else:
            ...
            alloc_df = pd.DataFrame({"Ticker": tickers, "MinVar": w_minv, "MaxSharpe": w_msr}).set_index("Ticker")
            st.dataframe(alloc_df.round(4))

            st.subheader("Max Sharpe Portfolio Allocation")
            st.plotly_chart(px.pie(alloc_df, values="MaxSharpe", names=alloc_df.index, title="Allocation (%)"))
